...
public class CustomScanner {
    ...
    private Boolean isConstant(String token) throws FileNotFoundException {
        FiniteAutomata FA_constant = new FiniteAutomata("C:\\Users\\Carmen\\Lab3\\fa\\constant");
        FA_constant.readFromFile();
        if(!FA_constant.isDFA()){
            return false;
        }

        FiniteAutomata FA_identifier = new FiniteAutomata("C:\\Users\\Carmen\\Lab3\\fa\\identifier");
    ...
    public void classifyTokens() throws IOException {
        ...
        PrintWriter pw = new PrintWriter("C:\\Users\\Carmen\\Lab3\\scanner_output\\pif");
        pw.printf("%-20s %s\n", "Token", "ST_Pos");
        ...
        if (!isStringLexicallyCorrect) {
            System.out.println("Lexical error: double quotes " + lastLine);
        }
        if (!isCharLexicallyCorrect) {
            System.out.println("Lexical error: quotes not closed at line " + lastLine);
        }
        pw.flush();
    }

    public void writeToSymbolTable() throws FileNotFoundException {
        PrintWriter pw = new PrintWriter("C:\\Users\\Carmen\\Lab3\\scanner_output\\st");
